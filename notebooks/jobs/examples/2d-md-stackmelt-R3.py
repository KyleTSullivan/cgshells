...
#     initial configuration loaded from final state of a previous emin job
#     will submit md runs at various temps and concentrations to run in parallel
#     runs md of energy minimized stack in a periodic box to determine melting temp
...
phi_list = [0.10]    # independent jobs that run in parallel (must be list)
temp_list = [0.025,0.05,0.075,0.1]    # more independent jobs that run in parallel (must be list)
...
for i in range(len(phi_list)):   
    ...
    #         force  = 0.1
            ...
            simpath = f"data/examples/{int(dimension)}d/md/stackmelt/nshells-{nshells}/phi-{phi:0.3f}/kT-{Tstart:0.3f}" # path to simulation directory (relative to PROJECT_ROOT)
    #         load_simpath = False # location of simulation to load in (set to False if not loading in state)
            load_simpath = f"data/examples/{int(dimension)}d/emin/kh-{kh:0.2f}/nshells-{nshells}" # location of simulation to load in (set to False if not loading in state)
