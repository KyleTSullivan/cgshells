...
nshells_list = np.arange(1,21,1).tolist()    # independent jobs that run in parallel (must be list)
kh_list = [1000]    # dependent jobs that run in series (must be list)
...
for i in range(len(nshells_list)):   
    ...
    for j in range(len(kh_list)):  
        ...
        r0 = 7.75 * dcore   # set to "flat" for particles with zero curvature
        ...
        k_i = 1.5 * k_0    # curvature of bottom shell in stack
        xlo = -2*wx
        xhi = 2*wx
        ylo = -4*r0
        yhi = nshells*r0 + 4*r0
