...
#     pre-stacked initial configuration
...
nshells_list = [2,4,6,8,10,12]    # independent jobs that run in parallel (must be list)
kh_list = [1.25,1.5,1.75,2.0]    # dependent jobs that run in series (must be list)
kh_loadlist = [1,1.25,1.5,1.75]    # start job from these final states (must be list)
minstyle_load = "cg"
etol_load = 1e-14
...
for i in range(len(nshells_list)):   
    ...
    for j in range(len(kh_list)):  
        ...
        a = 0.447 * dcore    # lattice constant of mesh
        mesh_name = f"a-{a:.3f}-wx-{wx:.2f}-wy-{wy:.2f}"    # name of bead-spring mesh to use for shell
        t0 = 2.0 * dcore    # structural thickness
        r0 = 20 * dcore #"flat"   # RMS radius of curvature; set to "flat" for planar plates 
...
#         kckh = 1 
        ...
        soft_ints = False
        sigma = 0.25 * dcore
        epsilon = 0.0002839415216518233 #4.783170983025395e-05 #0.00018472551620860115
        ...
        ##### SIMULATION #####
        ysep = 2.982 * dcore # t0 + dcore   # separation between shell midlines when stacking (ignore when loading config)
        config = "stacked" #"dispersed" or "stacked"
        simtype = "emin"
        datascript = "load3D"    # script to make data file with, NO .py EXTENSION, "stack3D", "ysep3D", "load3D"
        ...
        ki1 = 1.25 * k01    # principal curvatures for bottom shell in stack
        ki2 = 1.0 * k02
        theta_i = theta
...
#         force  = 0.1
        ...
        load_simpath = f"data/examples/{int(dimension)}d/emin/{mesh_name}/t0-{t0:0.3f}-r0-{r0:0.3f}/alpha_{alpha*180/np.pi:0.3f}/sigma-{sigma:0.5f}/kh-{kh_load:0.2f}/nshells-{nshells}/{minstyle_load}-{int(np.abs(np.log10(etol_load)))}" # location of simulation to load in (set to False if not loading in state)
