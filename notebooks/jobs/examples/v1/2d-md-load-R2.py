...
#     loads previous md run and continues it
#     will submit md jobs of various concentrations to run in parallel
#     best used for only continuing a job to larger number of steps
#       can change the number of cpus, requested runtime, runsteps, node
#       can't change anything else about simulation 
...
for i in range(len(phi_list)):   
    ...
    #         maxiter = 100000
            ...
            delete_existing = False    # if True, deletes simulation directory (and .sh files) if it exists before creating again
            simpath = f"data/examples/{int(dimension)}d/md/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/nshells-{nshells}/r0-{r0string}/sigma-{sigma:0.5f}-kh-{kh:0.3f}/phi-{phi:0.3f}/kT-{Tstart:0.3f}/damp-{damp:0.3f}" # path to simulation directory (relative to PROJECT_ROOT)
            #load_simpath = False # location of simulation to load in (set to False if not loading in state)
            load_simpath = f"data/examples/{int(dimension)}d/md/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/nshells-{nshells}/r0-{r0string}/phi-{phi:0.3f}/initial" # location of simulation to load in (set to False if not loading in state)
            ...
            tbuffer = 57 # stop lammps tbuffer minutes before walltime is exceeded
