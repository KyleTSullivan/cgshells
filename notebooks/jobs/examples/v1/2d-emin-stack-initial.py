...
#     pre-stacked initial configuration using curvature focusing rule
...
nshells_list = np.arange(1, 50+1).tolist()    # independent jobs that run in parallel (must be list) (list of different size curvemer stacks)
kh_list = [100]    # jobs that will run in series (must be list) (spring constants)
...
#         runsteps = 1*1000000
        ...
        simpath = f"data/examples/{int(dimension)}d/{simtype}/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{int(Nbeads)}/r0-{r0string}/sigma-{sigma:0.5f}/kh-{kh:0.5f}/nshells-{nshells}/{minstyle}-{int(np.abs(np.log10(etol)))}" # path to simulation directory (relative to PROJECT_ROOT)
