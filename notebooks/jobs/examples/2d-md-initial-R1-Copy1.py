...
phi_list = [0.25] #[0.05,0.10,0.15,0.20,0.25,0.3,0.35,0.4]    # independent jobs that run in parallel (must be list)
temp_list = [0.04]    # more independent jobs that run in parallel (must be list)
kh_list = [400]    # jobs that will run in series (must be list)
...
for i in range(len(phi_list)):   
    for j in range(len(temp_list)):
        ...
        for k in range(len(kh_list)):  
            ...
            r0 = "flat" #34 * dcore   # set to "flat" for particles with zero curvature
            ...
            simtype = "md"
            datascript = "lattice"    # script to make data file with, NO .py EXTENSION, "stack", "load", or "lattice"
            nshells = 50
            datagz = True
            trajgz = True
    ...
    #         maxiter = 100000
            ...
            seed = 15298
            timestep = 0.001
            runsteps = 6000000 #100000

            dumpfreq = 50000 #maxiter
            thermofreq = 1000
    ...
    #         force  = 0.1
            ...
            simpath = f"data/examples/{int(dimension)}d/md/phi-{phi:0.3f}/initial" # path to simulation directory (relative to PROJECT_ROOT)
