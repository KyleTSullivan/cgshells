...
sigma_list = [0.5,0.75]    # jobs that will run in series (must be list)
kh_list = ( 400 * np.ones( len(sigma_list) ) ).tolist() 
# start job from these final states (must be list)
sigma_loadlist = [0.25,0.5]
kh_loadlist = kh_list
minstyle_load = "cg"
etol_load = 1e-12
...
#         runsteps = 1*1000000
        ...
        sigma_load = sigma_loadlist[j]
#         load_simpath = False # location of simulation to load in (set to False if not loading in state)
        load_simpath = f"data/examples/{int(dimension)}d/emin/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{int(Nbeads)}/r0-{r0string}/sigma-{sigma_load:0.5f}/kh-{kh_load:0.5f}/nshells-{nshells}/{minstyle_load}-{int(np.abs(np.log10(etol_load)))}" # location of simulation to load in (set to False if not loading in state)
