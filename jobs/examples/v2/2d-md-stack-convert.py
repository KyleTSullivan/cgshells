...
#     loads initial state from previous stack simulation, converts 'nconvert' shell to moltype 2 and runs md 
#     will submit md jobs of various stack sizes and attractive strengths to run in parallel
...
for i in range(len(nshells_list)):   
    for j in range(len(epsilon2_list)):
        ...
        for k in range(len(kh_list)):  
            ...
            simtype = "md"
            datascript = "stack-convert"    # script to make data file with, NO .py EXTENSION, "stack", "load", or "lattice", etc.
            inputscript = "2d-2species-ljexpand-ramptemp"    # script to make lammps input file, NO .py EXTENSION   
            nshells = nshells_list[i]    # size of stack to import (total shells is nshells)
            datagz = True
            trajgz = True
            dumpbonds = False    # whether to calculate and dump bond data
            screen = True    # output lammps log to screen
            nconvert = nshells  # shell number in stack to convert to moltype 2
...
#             theta = "random"   # orientation of shells in lattice (0 = concave down, np.pi = concave up, "random" = randomly up or down)
            ...
            timestep = 0.0001
            runsteps = 10*1000000
            ramp_steps = 1*1000000

            dumpfreq = 10000 #maxiter
...
----------Deleting...""")
            ...
            if simtype == "md":
                mdparams = {'Tstart':Tstart,'Tstop':Tstop,'damp':damp,'seed':seed,
                            'timestep':timestep,'runsteps':runsteps,'ramp_steps':ramp_steps}
