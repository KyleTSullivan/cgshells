...
kh_list = yaml_data['initial_kh_preramp']    # jobs that will run in series (must be list)

jobcounter = 0

kh_list = yaml_data['subsequent_kh_preramp']
...
##############################################################################
#### determining enumerated strings of every possible stack configuration ####
##############################################################################

def tobinary(i, enumerated_iterator):
    binarystring = ''
    for location in range(i, 0, -1):
        #print(location)
        if (enumerated_iterator / (2 ** (location - 1))) >= 1:
            binarystring = '1' + binarystring
            enumerated_iterator = enumerated_iterator - (2 ** (location - 1))
...
uptolength = yaml_data['largest_stack']

binarylist = []
for l in range(uptolength):
    length = l + 1
    for value in range(2 ** length):
        print(value, length)
        binary = tobinary(length, value)
        print(binary)
        binarylist.append(binary)

listconfigs = binarylist

##############################################
####      generating curve selection      ####
##############################################

delta_k0 = yaml_data['deltak0'] #0.006, 0.012, 0.02 
k0bar = yaml_data['k0bar'] #k0bar: 0.08
Acurve = k0bar - (delta_k0 / 2)
Bcurve = k0bar + (delta_k0 / 2)

curvetypes = {'A': 1 / Acurve, 'B': 1 / Bcurve}
...
for sequence in listconfigs:
    ...
    for j in range(len(kh_list)): 
        ...
        ### Geometry
        dimension = 2
            # hard core diameter of beads (dcore approx thickness of one DNA helix 3.5nm)
...
#         runsteps = 1*1000000
        ...
        pattern = yaml_data['patternlist'][0]
        simpath = f"data/examples/{int(dimension)}d/{simtype}/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/species-{int(nspecies)}/pattern-{pattern}/r0-{r0string}/sigma-{sigma:0.5f}/kh-{kh:0.5f}/sequence-{curveseries}/nshells-{nshells}/{minstyle}-{int(np.abs(np.log10(etol)))}" # path to simulation directory (relative to PROJECT_ROOT)
        #load_simpath = False # location of simulation to load in (set to False if not loading in state)
        load_simpath = f"data/examples/{int(dimension)}d/{simtype}/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/species-{int(nspecies)}/pattern-{pattern}/r0-{r0string}/sigma-{sigma:0.5f}/kh-{khload:0.5f}/sequence-{curveseries}/nshells-{nshells}/{minstyle}-{int(np.abs(np.log10(etol)))}" # location of simulation to load in (set to False if not loading in state)         
...
----------Deleting...""")
    ...
    if computer != "local" and ((len(series_simpaths) % 25 == 0) or (simcounter == len(listconfigs))):

        simtrigger = simcounter
        #time.sleep(15)
        ##################################
        ...
        if simtype == "emin":
            shname = f"nshells-{nshells}-iteration-{simtrigger}"    
        elif simtype == "md":
