...
#curvetypes = yaml_data['curvetypes']
...
kh_list = yaml_data['subsequent_kh_preramp']    # jobs that will run in series (must be list)
# start job from these final states (must be list)
initialkh = yaml_data['initial_kh_preramp'][0]

if initialkh in kh_list:
    kh_list.remove(initialkh)

kh_loadlist = list(kh_list)
kh_loadlist.insert(0, yaml_data['initial_kh_preramp'][0])
kh_loadlist.pop()
minstyle_load = "hftn"
etol_load = 1e-12
...
for i in range(len(nshells_list)):
    ...
    for curvetypes in curvetypes_list:
        ...
        #         load_simpath = False # location of simulation to load in (set to False if not loading in state)
                ...
                if kh_load == initialkh:
                    load_simpath = f"data/examples/{int(dimension)}d/{simtype}/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/species-{int(nspecies)}/r0-{r0string}/sigma-{sigma:0.5f}/kh-{kh_load:0.5f}/nshells-{nshells}/{minstyle_load}-{int(np.abs(np.log10(etol)))}" # location of simulation to load in (set to False if not loading in state)
                else:
                    load_simpath = f"data/examples/{int(dimension)}d/{simtype}/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/species-{int(nspecies)}/pattern-{pattern}/r0-{r0string}/sigma-{sigma:0.5f}/kh-{kh_load:0.5f}/nshells-{nshells}/{minstyle_load}-{int(np.abs(np.log10(etol)))}" # location of simulation to load in (set to False if not loading in state)
