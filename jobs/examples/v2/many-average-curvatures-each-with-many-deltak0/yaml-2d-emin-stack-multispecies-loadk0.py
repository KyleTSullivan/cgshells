...
#     load initial stacked config from previous sim and perform energy minimization with different parameters
#     will submit various emin jobs that will run in parallel (dif. stack sizes) and in series (dif. spring constants)
...
def extendPattern(pattern, shells):
    patsize = len(pattern)
    curveserieslist = []
    output_extended_pattern = ''
    for particlesize in shells:
        remainder = particlesize % patsize
        #print(f"remainder: {remainder}")

        for i in range(int((particlesize - remainder) / patsize)):
            #print(int(particlesize / (patsize - remainder)))
            output_extended_pattern = output_extended_pattern + pattern

        if remainder != 0:
            for i in range(remainder):
                output_extended_pattern = output_extended_pattern + pattern[i]

        curveserieslist.append(output_extended_pattern)
        output_extended_pattern = ''

    return curveserieslist


with open('yaml.default', 'r') as defaultfile:
...
#curvetypes_list = yaml_data['curvetypes']
specieslist = yaml_data['species_list']
#curveseries_list = ['B', 'BA', 'BAA', 'BAAA', 'BAAAB', 'BAAABA', 'BAAABAA', 'BAAABAAA', 'BAAABAAAB', 'BAAABAAABB', 'BAAABAAABBA', 'BAAABAAABBAB', 'BAAABAAABBABB', 'BAAABAAABBABBA', 'BAAABAAABBABBAB', 'BAAABAAABBABBABB', 'BAAABAAABBABBABBA', 'BAAABAAABBABBABBAA', 'BAAABAAABBABBABBAAB', 'BAAABAAABBABBABBAABB', 'BAAABAAABBABBABBAABBB', 'BAAABAAABBABBABBAABBBB', 'BAAABAAABBABBABBAABBBBA', 'BAAABAAABBABBABBAABBBBAA', 'BAAABAAABBABBABBAABBBBAAB', 'BAAABAAABBABBABBAABBBBAABA', 'BAAABAAABBABBABBAABBBBAABAB', 'BAAABAAABBABBABBAABBBBAABABB', 'BAAABAAABBABBABBAABBBBAABABBB', 'BAAABAAABBABBABBAABBBBAABABBBA'] # manually setting a curveseries. Curveseries tells the curvamer generator which curve types to use in each location along a stack
patternlist = yaml_data['patternlist']
singlestacks = False
#kh_list = [100]    # jobs that will run in series (must be list)

##### SPRING CONSTANT LIST #####
kh_list = yaml_data['subsequent_kh']    # jobs that will run in series (must be list)
# start job from these final states (must be list)

kh_loadlist = list(yaml_data['subsequent_kh'])
kh_loadlist.insert(0, yaml_data['initial_kh'][0])
kh_loadlist.pop()
minstyle_load = "hftn"
etol_load = 1e-12
...
for i in range(len(nshells_list)):   
    ...
    # if (jobcounter % pauseinterval) == 0:
    #     time.sleep(pausetimesec)
    ...
    for j in range(len(kh_list)): 
        for curvetypes in curvetypes_list:
            ...
            for pattern in patternlist:
                curveseries_list = extendPattern(pattern, nshells_list) # automatically builds a curvseries list from a pattern
                for r in range(len(curvelist)):
                    simcounter += 1
                    ...
                    wx = yaml_data['wx_settings'] * dcore    # shell width (arclength along midline)
                    r0 = curvelist[r]   # set to "flat" for particles with zero curvature
                    loadr0 = curvelistload[r]   # set to "flat" for particles with zero curvature
                    Nbeads = yaml_data['Nbeads_settings']    # number of beads per layer (2Nbeads is beads per curvamer)
                    ...
                    if r0 == "flat":
                        ...
                        for key, value in loadr0.items():
                            loadr0string += str(key) + '=' + str(value)
                        isflat = False
                    ...
                    ### Interactions
                    nspecies = len(curvetypes_list[0])
                    pair_ints = "beta" #"none", "repulsive", "beta"
                    beta = 1 # fraction of total attractive energy that comes from mid patch;
                                        # 0 = outer flanks only; 1 = mid patch only (previous "1patch"); 
                                        # fraction = evenly split between mid and flanks (previous "patchy")
                    soft_ints = False
                    sigma = yaml_data['sigma_settings']*dcore
                    epsilon = yaml_data['epsilon_settings']
                    ...
                    simtype = "emin"
                    datascript = "load-change-multicurves"    # script to make data file with, NO .py EXTENSION, "stack", "load", or "lattice"
                    inputscript = "2d-1species-mixed-curves"    # script to make lammps input file, NO .py EXTENSION   
                    nshells = nshells_list[i]
                    curveseries = curveseries_list[i]
                    print(curveseries_list)
                    datagz = True
                    trajgz = True
            ...
            #         k_i = 1.25 * k_0    # independent of stack size
                    ...
                    avr0 = sum(list(r0.values())) / len(list(r0.values()))
                    if isflat:
                        k_i = 0
                    elif nshells == 1:
                        k_i = 1 / r0[curveseries]
                    else:    
                        k_0 = 1 / avr0
            ...
            #         runsteps = 1*1000000
                    ...
                    simpath = f"data/examples/{int(dimension)}d/emin/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{int(Nbeads)}/fraction-{fraction:0.3f}/species-{len(specieslist)}/pattern-{pattern}/k0bar-{str(((1 / r0['A']) + (1 / r0['B'])) / 2)}/r0-{r0string}/sigma-{sigma:0.5f}/kh-{kh:0.5f}/nshells-{nshells}/{minstyle}-{int(np.abs(np.log10(etol)))}" # path to simulation directory (relative to PROJECT_ROOT)
                    kh_load = kh_loadlist[j]
                    print(kh_load)
            #         load_simpath = False # location of simulation to load in (set to False if not loading in state)
                    load_simpath = f"data/examples/{int(dimension)}d/{simtype}/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/species-{int(nspecies)}/pattern-{pattern}/k0bar-{str(((1 / loadr0['A']) + (1 / loadr0['B'])) / 2)}/r0-{loadr0string}/sigma-{sigma:0.5f}/kh-{kh_load:0.5f}/nshells-{nshells}/{minstyle_load}-{int(np.abs(np.log10(etol)))}" # location of simulation to load in (set to False if not loading in state)
                    ...
                    py = 1
                    pz = 1
                    #pmax = 2    # max number of cpus per job along y axis (does not exceed pmax)
                    # nchunk = 5    # increase number of cpus by 1 every nchunk shells
                    # if nshells<=nchunk*(pmax-1):
                    #     py = int(np.floor((nshells-1)/(nchunk))+1)
                    # else:
                    #     py = int(pmax)
