        ...
        for curvetypes in curvetypes_list:
            for pattern in patternlist:
                ...
                for r in range(1):
                    ...
                    r0 = curvetypes   # set to "flat" for particles with zero curvature
                    loadr0 = curvetypes   # set to "flat" for particles with zero curvature
            ...
            #         runsteps = 1*1000000
                    ...
                    simpath = f"data/examples/{int(dimension)}d/emin/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{int(Nbeads)}/fraction-{fraction:0.3f}/species-{len(specieslist)}/pattern-{pattern}/r0-{r0string}/sigma-{sigma:0.5f}/kh-{kh:0.5f}/nshells-{nshells}/{minstyle}-{int(np.abs(np.log10(etol)))}" # path to simulation directory (relative to PROJECT_ROOT)
                    kh_load = kh_loadlist[j]
                    print(kh_load)
            #         load_simpath = False # location of simulation to load in (set to False if not loading in state)
                    load_simpath = f"data/examples/{int(dimension)}d/{simtype}/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/species-{int(nspecies)}/pattern-{pattern}/r0-{loadr0string}/sigma-{sigma:0.5f}/kh-{kh_load:0.5f}/nshells-{nshells}/{minstyle_load}-{int(np.abs(np.log10(etol)))}" # location of simulation to load in (set to False if not loading in state)
