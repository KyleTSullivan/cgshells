...
#SBATCH -o /work/pi_grason_umass_edu/kboisjolie/cgshells/jobs/examples/v2/2d-emin-stack-initial-multi-curvetypes-sbatch/nshells-24-%j.out # %j = job ID
...
series_simpaths = ["data/examples/2d/emin/wx-4.900-t0-0.600-Nbeads-15/fraction-0.333/species-1/r0-{'A': 6.5}/sigma-0.25000/kh-100.00000/nshells-24/cg-12"]
rc_max = [1]
...
if len(simpaths_torun) > 0:
    ...
    if restartjob:
        if (rc < rcmax):
            ...
            sbatch = subprocess.run(['sbatch','/work/pi_grason_umass_edu/kboisjolie/cgshells/jobs/examples/v2/2d-emin-stack-initial-multi-curvetypes-sbatch/nshells-24.sh'],
