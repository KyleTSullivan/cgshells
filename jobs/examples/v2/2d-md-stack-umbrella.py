...
#     loads initial state from previous stack simulation, adds new particle to box and runs md 
...
nshells_list = [1]  # independent jobs that run in parallel (must be list)
ysep_list = [1.5,1.4,1.3,1.2,1.1,1.0]  # more independent jobs that run in parallel (must be list)
...
for i in range(len(nshells_list)):   
    for j in range(len(ysep_list)):
        ...
        for k in range(len(kh_list)):  
            ...
            ##### SIMULATION #####
            ysep = ysep_list[j]*(t0+dcore)  # center-center separation distance between top most shell and new shell
            kbias = 100    # spring constant of harmonic bias potential
            ...
            inputscript = "2d-1species-umbrella"    # script to make lammps input file, NO .py EXTENSION   
            ...
            k_i = 0 * k_0    # initial curvature of additional shell
    #         xlo = -2*wx
    #         xhi = 2*wx
    #         ylo = -4*r0
    #         yhi = nshells*r0 + 4*r0
...
#             theta = "random"   # orientation of shells in lattice (0 = concave down, np.pi = concave up, "random" = randomly up or down)
            ...
            nshells_fict = 200    # phi = 1 box volume has same vol. as nshells_fict shells (smaller box for smaller nshells_fict)
            phi = 1    # concentration of molecules (area fraction) - only for MD
            ...
            simpath = f"data/examples/{int(dimension)}d/md/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/nshells_fict-{nshells_fict}/species-{int(nspecies)}/r0-{r0string}/ljexpand-beta-{beta:0.3f}/sigma-{sigma:0.5f}-kh-{kh:0.3f}/phi-{phi:0.3f}/kT-{Tstop:0.3f}/damp-{damp:0.3f}/stack-{nshells}/kbias-{kbias:0.3f}/ysep-{ysep/(t0+dcore):0.3f}" # path to simulation directory (relative to PROJECT_ROOT)
...
----------Deleting...""")
            ...
            ##########################                       
            nids = []
            if Nbeads%2 == 1:    # odd Nbeads -> com of nearest two unit cells to shell center
                nids.append(int(Nbeads/2))
                nids.append(int(Nbeads/2)+1)
                nids.append(int(Nbeads/2)+2)
                nids.append(int(Nbeads/2)+Nbeads)
                nids.append(int(Nbeads/2)+1+Nbeads)
                nids.append(int(Nbeads/2)+2+Nbeads)
                
            else:    # even Nbeads -> com of center most unit cell
                nids.append(int(Nbeads/2))
                nids.append(int(Nbeads/2)+1)
                nids.append(int(Nbeads/2)+Nbeads)
                nids.append(int(Nbeads/2)+1+Nbeads)             
            ...
            shell1 = nshells + 1
            group1nids = np.array(nids)+(shell1-1)*2*Nbeads
            group1str = ""
            for ni in range(len(group1nids)):
                group1str += f" {group1nids[ni]}"
            
            shell2 = nshells 
            group2nids = np.array(nids)+(shell2-1)*2*Nbeads
            group2str = ""
            for ni in range(len(group2nids)):
                group2str += f" {group2nids[ni]}"

            colvar_contents = f"""

colvarsTrajFrequency {thermofreq}
...
colvar {{
    ...
    distance {{
        ...
        group1 {{
            atomNumbers {{ {group1str} }}
        }}

        group2 {{
            atomNumbers {{ {group2str} }}
        }}
    }}
}}
...
"""
            with open(f"{PROJECT_ROOT}/{simpath}/colvars.in", "w") as f:
                f.write(colvar_contents)
