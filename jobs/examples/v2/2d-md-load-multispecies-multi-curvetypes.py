...
#     loads initial state from previous simulation and runs md 
...
temp_list = [0.04]    # more independent jobs that run in parallel (must be list)
...
for i in range(len(phi_list)):   
    for j in range(len(temp_list)):
        ...
        for k in range(len(kh_list)):  
            ...
            #r0 = [5.0 * dcore, 8.0 * dcore]   # set to "flat" for particles with zero curvature
            r0 = ['flat', 'flat', 'flat']   # set to "flat" for particles with zero curvature
            weightedaveragecurves = [0.33, 0.33, 0.34] # weighted average between listed curves in r0. Must add up to one.
            labels = ['A', 'B', 'C']
            allowedattractions = ['A-A', 'A-B', 'B-B', 'C-C']
    ...
    #         maxiter = 100000
            ...
            seed = 15298
            timestep = 0.0001
            runsteps = 100*1000000

            dumpfreq = 100000 #maxiter
            thermofreq = 1000#dumpfreq

            ##### LOGISTICS #####

            ### Simulation Directories
            delete_existing = True    # if True, deletes simulation directory (and .sh files) if it exists before creating again
            simpath = f"data/examples/{int(dimension)}d/md/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/nshells-{nshells}/species-{int(nspecies)}/r0-{r0string}/sigma-{sigma:0.5f}-kh-{kh:0.3f}/beta-{beta:0.3f}/phi-{phi:0.3f}/kT-{Tstart:0.3f}/damp-{damp:0.3f}" # path to simulation directory (relative to PROJECT_ROOT)
            #load_simpath = False # location of simulation to load in (set to False if not loading in state)
            load_simpath = f"data/examples/{int(dimension)}d/md/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/nshells-{nshells}/species-{int(nspecies)}/r0-{r0string}/phi-{phi:0.3f}/initial"  # location of simulation to load in (set to False if not loading in state)
