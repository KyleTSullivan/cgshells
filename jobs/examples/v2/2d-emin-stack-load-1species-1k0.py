...
kh_list = [1200]    # jobs that will run in series (must be list)
sigma_list = [0.25]
...
#         load_simpath = False # location of simulation to load in (set to False if not loading in state)
        load_simpath = f"data/examples/{int(dimension)}d/{simtype}/wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}/fraction-{fraction:0.3f}/species-{int(nspecies)}/r0-{r0string}/sigma-{sigma_load:0.5f}/kh-{kh_load:0.5f}/nshells-{nshells}/{minstyle_load}-{int(np.abs(np.log10(etol_load)))}"
