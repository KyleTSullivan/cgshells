...
kh_list = [1]    # dependent jobs that run in series (must be list)
...
for i in range(len(nshells_list)):   
    ...
    for j in range(len(kh_list)):  
        ...
        theta = 0    # angle principal direction 1 make_simpaths_files with material x-axis
...
#         kckh = 1 
        ...
        ##### SIMULATION #####
        ysep = 2.982 * dcore # t0 + dcore   # separation between shell midlines when stacking
        config = "stacked" #"dispersed" or "stacked"
        simtype = "emin"
        datascript = "ysep3D"    # script to make data file with, NO .py EXTENSION, "stack3D", "ysep3D", "load3D"
