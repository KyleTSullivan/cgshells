# stack_distributions.py

# given path to simulation directory, 
    # read trajectory files and save aggregate size distributions to txt file
...
def write_dists(simpath):
    ...
    if r0 == "flat":
        ...
        letterlist = ['A', 'B']
        for value in r0:
            valueindex = r0.index(value)
            r0string += letterlist[valueindex] + '=' + str(value)
        isflat = False

    if r0 == "flat":
        txtfilename = f"wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}-nshells-{nshells}-r0-{r0string}-sigma-{sigma:0.3f}-kh-{kh:0.3f}-pair_ints-{pair_ints}-soft_ints-{soft_ints}-phi-{phi:0.4f}-kT-{kT:0.4f}-damp-{damp:0.2f}-ethresh-{float(np.abs(ethresh)):0.3f}.txt"
    else:
        txtfilename = f"wx-{wx:0.3f}-t0-{t0:0.3f}-Nbeads-{Nbeads}-nshells-{nshells}-r0-{r0string}-sigma-{sigma:0.3f}-kh-{kh:0.3f}-pair_ints-{pair_ints}-soft_ints-{soft_ints}-phi-{phi:0.4f}-kT-{kT:0.4f}-damp-{damp:0.2f}-ethresh-{np.abs(ethresh):0.3f}.txt"
