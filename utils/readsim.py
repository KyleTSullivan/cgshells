...
#             pass
        ...
        # read dump file (trajectory of atoms)
...
#             setattr(self,"dump_"+self.dump_labels[i],data)
    ...
    # read dump file (can read both atom trajectory and other dump types, e.g bonds)
    def read_dump(self,dumpname="dump.lammps",readall=False):
        ...
        # see if dump is trajectory of atoms or other
        try:
            search = "ATOMS"
            end_index = in_text.index(f"ITEM: {search} ")
        ...
        # find number of atoms in simulation
        start_index = in_text.index(f"ITEM: NUMBER OF {search}") + len(f"ITEM: NUMBER OF {search}")
        end_index = in_text.index("ITEM:", start_index)
        if search == "ATOMS":
            self.natoms = int(in_text[start_index : end_index])
            self.nentries = self.natoms
        else:
            self.nentries = int(in_text[start_index : end_index])

        # fetch data column labels
        start_index = in_text.index(f"ITEM: {search} ") + len(f"ITEM: {search} ")
        end_index = in_text.index("\n",start_index)
        self.dump_labels = in_text[start_index : end_index].split()

        
        # gather data dumps
        self.nframes = linecount//(toplines+self.nentries)    # number of completed dumps in file
        tsteps = []
        dumps = []
        ...
        self.tstep_i = int(in_text[ts:tf])
        tsteps.append(self.tstep_i)
        d0 = in_text.index(f"ITEM: {search} ") + len(f"ITEM: {search} ")
        ds = in_text.index("\n",d0)+1
        if self.nframes == 1:
            self.dump_i = np.fromstring(in_text[ds:],sep='\n').reshape(self.nentries,len(self.dump_labels))
            self.dump_f = self.dump_i
        else:
            df = in_text.index("ITEM: TIMESTEP\n",tf)-1
            self.dump_i = np.fromstring(in_text[ds:df],sep='\n').reshape(self.nentries,len(self.dump_labels))
        ...
        for fn in (np.arange(1,self.nframes)):
            ...
            tf = in_text.index("\n",ts)
            tstep_n = int(in_text[ts:tf])
            d0 = in_text.index(f"ITEM: {search} ",ts) + len(f"ITEM: {search} ")
            ds = in_text.index("\n",d0)+1
