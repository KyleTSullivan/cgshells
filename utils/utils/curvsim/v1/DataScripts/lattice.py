...
def make_data(simpath):
    ...
    if theta == "random":
        ...
        for i in range(int(nshells)):
            sim.make_curvamer(rxlist[i],rylist[i],np.random.randint(0,2)*theta,wx,Nbeads,fraction,t0,k_0,k_i,kh,kckh,kvkh)
    else:
        for i in range(int(nshells)):
            sim.make_curvamer(rxlist[i],rylist[i],theta,wx,Nbeads,fraction,t0,k_0,k_i,kh,kckh,kvkh)
