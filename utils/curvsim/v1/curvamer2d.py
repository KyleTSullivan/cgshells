...
class Curvamer2D:
    ...
    def make_curvamer(self,rx,ry,theta,w,N,fraction,t0,k_0,k_i,kh,kckh,kvkh):
...
#         middlemask = (xflat>=(-w/2 + w*(fraction-1)/2)) & (xflat<=(w/2 - w*(fraction-1)/2))
        atomtypes[bottommask*leftmask] = 1
        atomtypes[bottommask*rightmask] = 1
        atomtypes[bottommask*middlemask] = 3
        atomtypes[topmask*leftmask] = 2
        atomtypes[topmask*rightmask] = 2
        atomtypes[topmask*middlemask] = 4
...
#         mass = 1/natoms
        ...
        self.nmols += 1
        self.natoms += natoms
        self.natomtypes = 4 #np.size(np.unique(atomtypes))
        self.nbonds += nbonds
    ...
    def run_flat_adhesion(self,ysep,dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma):
        """Calculates energy between 2 flat curvamers separated by midline-midline distance ysep"""
        k_0 = 0    # preferred curvature
        k_i = 0    # initial curvature
        kh = 0     # spring constant
        kckh = 1
        kvkh = 1
        ncurvamers = 2
        shift = dcore - 2**(1/6)*sigma 
        ljcut = 5*sigma #t0 + 2*dcore # make cutoff dists fxn arguments
        wcacut = dcore
        softsigma = 2*sigma
        
        etol = 1e-10        # both of these are arbitrary as forces set to zero (no effect if changed)
        maxiter = 100000    
        ...
        xhi = rx + 1*wx
        ylo = ry - 1*(t0+dcore)
        yhi = ry + (ncurvamers + 0.5 + 5)*(t0+dcore)

        simpath = "{}/FlatAdhesion".format(self.directory)
        sim = Curvamer2D(directory = self.directory)
        for nmol in np.arange(ncurvamers):
            rx_n = rx
            ry_n = ry + nmol*ysep
        #     r_n = r_i + nmol*(t0+dcore)
            k_n = k_i
            sim.make_curvamer(rx_n,ry_n,wx,nbeads,patch_fraction,t0,k_0,k_n,kh,kckh,kvkh)
        sim.make_datafile(simpath,xlo,xhi,ylo,yhi,zlo=-0.5,zhi=0.5)

        with open("{}/in.lammps".format(simpath),'w') as fdata:
            fdata.write('# 2D Curvamer Input File - Bending Test\n')
            fdata.write("\nunits lj")
            fdata.write("\ndimension 2")
            fdata.write("\nboundary f f p")
        ...
        #     fdata.write("\nspecial_bonds lj 0.0 0.0 0.0\n")
            ...
            dump1 = "\ndump 1 all custom 500 {}/dump.lammps mol id type x y z".format(simpath)
        ...
        readsim = ReadSim(simpath)
        readsim.read_log_emin()
        return readsim.energyf

    def find_gammaA(self,dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma):
        params = [dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma]
        ### Golden ratio search for energy minimum
        accuracy = 1e-3         # required accuracy to stop search
        z = (1+np.sqrt(5))/2    # golden ratio

        # initial positions of search
        x1 = 0.9*(t0+dcore)
        x4 = 1.1*(t0+dcore)
        x2 = x4 - (x4-x1)/z
        x3 = x1 + (x4-x1)/z
        ...
        sim = Curvamer2D(directory = self.directory)
        f1 = sim.run_flat_adhesion(x1,*params)
        f2 = sim.run_flat_adhesion(x2,*params)
        f3 = sim.run_flat_adhesion(x3,*params)
        f4 = sim.run_flat_adhesion(x4,*params)
        ...
        # result
        xmin = 0.5*(x1+x4)
        fmin = sim.run_flat_adhesion(xmin,*params)
        if (loops == 100):
            print("exceeded max iterations (100) in search for minimum")
        return xmin, fmin, loops 
    
    def find_gppA(self,dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma,t):
        """ faulty method.  Use fit_gppA() """

        h = 1e-4    # step size
        yseps = np.array([t-h,t,t+h])  # midline-midline separation
        params = [dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma]
        
        energies = []
        sim = Curvamer2D(directory = self.directory)
        for ysep in yseps:
            energies.append(sim.run_flat_adhesion(ysep,*params))

        fx = energies[1]
        fxplus = energies[2]
        fxminus = energies[0]
        gppA = (fxplus - 2*fx + fxminus) / (h**2)
        return gppA
    
    def fit_gppA(self,dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma,t):
        
        sim = Curvamer2D(directory = self.directory)
        yseps_2 = np.linspace(t-0.02*sigma,t+0.02*sigma,20)
        energies_2 = []
        for ysep in yseps_2:
            energies_2.append(sim.run_flat_adhesion(ysep,dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma))
        energies_2 = np.array(energies_2)
        
        def harmonic(x,e0,epp):
            return -e0 + 0.5*epp*(x-1)**2

        params, cov = curve_fit(harmonic, yseps_2/t,energies_2)
        gppA = params[1]
        
        return gppA

    def run_flat_adhesion_alt(self,ysep,dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma):
        """Calculates energy between 2 flat curvamers separated by midline-midline distance ysep"""
        k_0 = 0    # preferred curvature
        k_i = 0    # initial curvature
        kh = 0     # spring constant
    ...
    def find_gammaA_alt(self,dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma):
        params = [dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma]
        ### Golden ratio search for energy minimum
        accuracy = 1e-3         # required accuracy to stop search
        z = (1+np.sqrt(5))/2    # golden ratio
        ...
        # initial values of the function at the initial points
        sim = Curvamer2D(directory = self.directory)
        f1 = sim.run_flat_adhesion_alt(x1,*params)
        f2 = sim.run_flat_adhesion_alt(x2,*params)
        f3 = sim.run_flat_adhesion_alt(x3,*params)
        f4 = sim.run_flat_adhesion_alt(x4,*params)
        ...
        # result
        xmin = 0.5*(x1+x4)
        fmin = sim.run_flat_adhesion_alt(xmin,*params)
        if (loops == 100):
            print("exceeded max iterations (100) in search for minimum")
        return xmin, fmin, loops 
    
    def find_gppA_alt(self,dcore,wx,nbeads,patch_fraction,t0,epsilon,sigma,t):
        ...
        sim = Curvamer2D(directory = self.directory)
        for ysep in yseps:
            energies.append(sim.run_flat_adhesion_alt(ysep,*params))
    ...
    def find_BA(self,dcore,wx,nbeads,patch_fraction,t0,r0,kh,kckh,kvkh,etol,maxiter):
        """Calculates flattening energy of a curvamer with horizontal spring constant kh"""
        k_0 = 1/r0    # preferred curvature
        k_i = 0    # initial curvature
        ncurvamers = 1
        ...
        xlo = rx - 1*wx
        xhi = rx + 1*wx
        ylo = ry - 1*(t0+dcore) - r0
        yhi = ry + (t0+dcore) + r0
        
        simpath = "{}/BendingModulus".format(self.directory)
        sim = Curvamer2D(directory = self.directory)
        for nmol in np.arange(ncurvamers):
            rx_n = rx
            ry_n = ry 
    ...
    #         fdata.write("\npair_modify shift yes\n")
            ...
            fdata.write("\nfix 1 all enforce2d\n")
            fdata.write("\nfix 2 all setforce NULL 0.0 0.0\n")    # constrain to x movements only (set y forces to zero)
        ...
        readsim = ReadSim(simpath)
        readsim.read_log_emin()
        E_flat = readsim.energyf
        BA = 2*E_flat/(k_0**2) 
        return E_flat, BA
    
    def find_BA_alt(self,dcore,wx,nbeads,patch_fraction,t0,r0,kh,kckh,kvkh,etol,maxiter):
        """Calculates flattening energy of a curvamer with horizontal spring constant kh"""
        k_0 = 1/r0    # preferred curvature
        k_i = 0    # initial curvature
        ncurvamers = 1
