# 2d-2species-ljexpand-ramptemp.py
...
# for use with stack addition free energy calculation (lj/expand potential)
    ...
    # ramptemp version allows for temperatures to be slowly ramped up from zero
...
def make_input(simpath):
    ...
    if simtype == "md":
        ...
        ramp_steps = meta['simulation']['ramp_steps']
...
# ---------- Heating ----------
if "${{stage}} == 1" then "fix 1 all langevin {Tstart} {Tstop} {damp} {seed}"
if "${{stage}} == 1" then "run {ramp_steps} upto"

# ---------- Constant Temp -----------
if "${{stage}} > 1" then "fix 1 all langevin {Tstop} {Tstop} {damp} {seed}"
if "${{stage}} > 1" then "run {runsteps} upto"
