# ysep.py
# makes stack of curvamers with distance ysep between their midlines
# for calculating adhesive energies
...
def make_data(simpath):
    ...
    orientation = meta['simulation']['orientation']
    ysep = meta['simulation']['ysep']
    curveseries = meta['particle']['geometry']['curveseries']
    if "flat" in r0.values():
        k_0 = 0
        isnotflat = False
    else:
        #k_0 = 1/r0
    ...
    print("Using ysep.py to create data file...")
    print("nshells = {}".format(nshells))
    print("...preparing new simulation data")
    theta = np.pi    # orientation of stack (0 = concave down)
    x_i = 0    # location of bottom molecule
    y_i = 0
    sim = Curvamer2D(directory=f"{PROJECT_ROOT}/{simpath}")

    curve_identities = list(curveseries)
    print(curve_identities)

    for n in range(int(nshells)):

        rx_n = x_i    # x pos of nth molecule
        ry_n = y_i + n * ysep    # y pos of nth molecule
        moltype_i = 1

        print("before if")
        if (isnotflat):
            print("after if")
            curvetag = curve_identities[n]
            print(curvetag)
            specificr0 = r0[curvetag]
            print(specificr0)
            k_0 = 1 / specificr0
